...
if(!empty($_POST['oldpass']) && !empty($_POST['newpass']) && !empty($_POST['newpass2'])) {
    ...
    if($_POST['newpass'] === $_POST['newpass2']) {
        // récupération de l'utilisateur en BDD pour comparaison du mot de passe
        $query = $connect->prepare("SELECT * FROM demandeur WHERE id= ?");
        $query->execute([$_SESSION['id']]);
        $demandeur = $query->fetch();
        // verification ancien mot de passe
        $verif = password_verify($_POST['oldpass'], $demandeur['pass']);
        if($verif ===true) {
            // cryptage MDP
            $pass = password_hash($_POST['newpass'], PASSWORD_BCRYPT);
            // mise à jour MDP si toutes les verifs sont bonnes
            $query = $connect->prepare("UPDATE demandeur SET pass = ? WHERE id = ?");
