...
if (!empty($_POST['job']) && !empty($_POST['pseudo']) && !empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['compagny_name']) && !empty($_POST['mail']) && !empty($_POST['pass']) && !empty($_POST['pass2'])) {
    ...
    if ($_POST['pass'] === $_POST['pass2']) {
        ...
        $compagny_name = htmlspecialchars($_POST['compagny_name']);
        $mail = htmlspecialchars($_POST['mail']);
        $pass = password_hash($_POST['pass'], PASSWORD_BCRYPT);
        //  verifier si mail ou pseudo existent
        $query = $connect->prepare("SELECT * FROM freelance WHERE pseudo = ? OR mail = ?");
        $query->execute([$pseudo, $mail]);
        $response = $query->fetch();
        if($response === false) {
            // inscription de l'utilisateur si le mail ou le pseudo n'existent pas
            // insertion dans la BDD
            $query = $connect->prepare("INSERT INTO freelance(job, pseudo, first_name, last_name, compagny_name, mail, pass, nb_projects, note) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $query->execute([$job, $pseudo, $first_name, $last_name, $compagny_name, $mail, $pass, $nbprojects, $note]);
